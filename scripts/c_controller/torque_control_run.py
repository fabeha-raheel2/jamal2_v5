...
class QuadrupedController:
            ...
            if self._debug:
                ...
                if user_input == "y" or user_input == "Y":
                    for motor in self.motors.values():
                        ...
                        self.last_joint_positions.append(0)
    ...
    def run_custom_torque_controller(self):
        while not rospy.is_shutdown():
            ...
            for motor, position in zip(self.motors.values(),self.joint_positions):
                ...
                try:
                    torque_cmd = KP_IN * (motor.adjust(position) - self.last_joint_positions[self.joint_names.index(motor.name)])
                    feedback = self.pcan_bus.send_motor_data(motor_id=motor.id, pos=0, v_in=0, kp_in=0, kd_in=3.0, t_in=torque_cmd)
                    self.feedback_positions.append(motor.readjust_position(feedback['position']))
                    self.joint_names.append(motor.name)
                    current_positions.append(feedback['position'])
...
if __name__ == "__main__":
    ...
    quadruped_controller.run_custom_torque_controller()